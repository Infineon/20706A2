...
**
**  Description: This file contains common defines / structure and structures used by Encoder / Decoder
**
******************************************************************************/
...
1.30        |        First version with mSBC floating frame support inside the library
1.31        |        - Update WBS trace decoder for PC application
            |        - Improve BEC2 MHz (Peak WBS + PLC + BEC is 7.1 MHZ)
            |
1.32        |        - Add Lip Sync
            |        - Change PLRATT_MAX default value to 4096 <=> 0.125
            |        - Remove 16 kHz Sample Rate Converter        |
            |
1.33        |        - Add BTW support
            |
1.34        |        - Add SBC Encoder for A2DP for BTW
            |
1.35        |        - Add CVSD SW Re encoding for ARM7
            |
1.36        |        - Fix issue with EV3 Packet Type
            |        - Fix interface issue between Fw / library:
            |        GOOD_PACKET is not necessary first mSBC frame
1.37        |        - Add Independant contextfor BTW
            |
1.38        |        - Fix BEC2 threshold in Ref C Code
            |        - Change BEC2 threshold to 100 per Rob Z mail (09/30/2011)
...
The latter is protected by a simple repetition code (both bits are duplicated).
Hence, each pair of bits in the sequence number shall be always 00 or 11. */
