...
#ifndef __PCM_DRIVER_H___
#define __PCM_DRIVER_H___
...
struct BCS_SLOT_CB_ENTRY;

void pcmSlotCallback(BCS_SLOT_CB_ENTRY *slotCbEntry);
...
class Pcm : public StartupAllocated
{
public:
...
protected:
    ...
    void init(void);
    void intConfig(UINT32 pcmWaterlineLevel, void (*func)(UINT32));
...
public:
    static Pcm* (*createInstance)(void);
    static Pcm* defaultCreateInstance( void );
    static Pcm* getInstance(void);

    //  pcm Reset PCM module
    void reset(void);

    //  Kick off  PCM in master mode after the PCM module is properly configured.
    void start(CFG_TYPE* config , void (*pFunc)(UINT32));


    // eye candy for applcaiton class
    void stop(void)
    {
        reset();
    }
    ...
    virtual void interuptEnable(void);
    ...
    // The Buffer read length should be less than the valid data in FIFO
    UINT8 bufferRead(UINT16* readBuff,UINT16 buffLength,BYTE channelIndex);


    //  pcm Buffer write
    UINT8 bufferWrite(UINT16* WriteBuff,UINT16 buffLength,BYTE channelIndex);


    //  Get number of Empty space in PCM FIFO
    UINT16 getEmptySpace(BYTE chIndex);


    //  Get the number of Valid data entries in PCM FIFO
    UINT16 getValidDataCount(BYTE chIndex);


    //  pcm Interupt Service Routine
    int handlePcmInterrupt(void* unused);

protected:

    volatile CONTROL_ADR_TYPE *control;
    volatile CONTROL_2_ADR_TYPE *control2;
    volatile MISC_CTRL_ADR_TYPE *miscCtrl;

    //PCM interrupt callback function pointer
    void (*pcmCallback)(UINT32);

    //  slot callback function
    friend void pcmSlotCallback(BCS_SLOT_CB_ENTRY *slotCbEntry);
