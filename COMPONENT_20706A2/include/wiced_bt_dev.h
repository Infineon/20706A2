 ...
 *
 * @param[in]       first        : first channel to disable
 * @param[in]       last         : last channel to disable
 *
 * @return          TRUE if successful
 *
 */
...
/** Paired device link key notification (used by BTM_PAIRED_DEVICE_LINK_KEYS_UPDATE_EVT notification) */
typedef struct
{
    wiced_bt_device_address_t  bd_addr;                                     /**< [in] BD Address of remote */
    wiced_bt_device_sec_keys_t key_data;         /**< [in/out] Key data */
} __attribute__((packed)) wiced_bt_device_link_keys_t;
...
enum
{
    WICED_TRANS_CUSTOM_HCI_MODE,
    WICED_TRANS_RAW_MODE,
};
...
/**
 *
 * This section talks about the various API's required to integrate BR/EDR functionality
 ...
 *
 * @param[in]       inq_mode              : inquiry mode(Ex: BTM_INQ_RESULT_EXTENDED)
 *
 * @return          wiced_result_t
 *
 *                  WICED_BT_SUCCESS            if successful
 ...
 *
 * @param[in]       scan_type        : BTM_SCAN_TYPE_STANDARD or BTM_SCAN_TYPE_INTERLACED
 *
 * @return
 *
 *                  WICED_BT_SUCCESS:        If successful
 ...
 *
 * @param[in]       timeout        : Page Timeout value in Number of Baseband slots (1 baseband slot = 0.625 ms)
 *
 * @return
 *
 *                  WICED_BT_SUCCESS:        If successful
 ...
 * @param[in]       pkt_types           : baseband packet types. see HCI_PKT_TYPES_MASK_* definitions in wiced_bt_hci_defs.h
 *
 * @return
 *
 ...
 *
 * @param[in]       remote_bda      : device address of desired connection
 *
 * @param[out]     p_mode - address where the current mode is copied into.
 *                          WICED_POWER_STATE_ACTIVE
 *                          WICED_POWER_STATE_SNIFF
 *                          WICED_POWER_STATE_SMART_SNIFF
 *                          (valid only if return code is WICED_SUCCESS)
 *
 * @return          WICED_SUCCESS if successful,
 *                       WICED_ERROR if bd addr is not active or bad
 *
 */
 ...
 *
 *                  Note:  see Bluetooth Core Spec definition of the Read RSSI command, for BR/EDR transports,
 *                         the RSSI value returned through the callback will be the difference between the measured
 *                         RSSI and the limits of a range selected by the controller, where positive or negative
 *                         values indicate a measurement above or below the range, and a zero value indicates the
 *                         RSSI is within the range.  But for LE transports, the value returned will be the absolute
 *                         RSSI.  If absolute RSSI is required for a BR/EDR connection, use the wiced_bt_read_raw_rssi()
 ...
 *
 *                  Remove link key information from internal Security Records.
 *                  NOTE: This API does not remove key info if device is in connected
 *                  state (it would return FALSE), hence existing connections should be
 *                  disconnected before invoking this API
 *
 * @param[in]       bd_addr         : peer address
 *
 * @return          TRUE if successful
